/* A simple expandable array implementation.
   ...
   * NOTE: Because of the fact that this expandable array
     ...
     programming situations where macro inputs get clobbered or
     evaluated twice during intermediate execution of the macros.  So
     far, this code was only modified in one place to compensate for
     these issues.
...
/* Remove a range of elements at index `pos' of length `len'.  The
   allocated array memory is not shrunken.
...
/* Remove all elements starting at a zero-based index and going until
   ...
   `EA_REMOVE_RANGE()'.  */
#define EA_ERASEP(typename, array, pos) \
	EA_REMOVE_RANGE(typename, array, pos, (array).len - pos)
...
/* Remove a range of elements using a zero-based index to the first
   ...
   element to remove.  */
#define EA_ERASER(typename, array, begin, end) \
	EA_REMOVE_RANGE(typename, array, begin, end - begin)
